...
2. `impute_median` / `impute_mean` / `impute_mode`: Impute missing values using domain statistics.
3. `fill_value`: Replace nulls with an explicit default (e.g. 0.0 for missing discount, 'NONE' for promo code).
...
class NullHandler:
    ...
    def __init__(self, strategies: dict[str, dict[str, Any]]) -> None:
        ...
        Example:
            strategies = {
                ...
                'churn_risk_score': {'action': 'impute_median'},
                'promo_code': {'action': 'fill_value', 'value': 'UNSPECIFIED'},
            }
        """
    ...
    def process(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info("Executing explicit null-handling strategies across %d columns...", len(df.columns))
        ...
        for col, spec in self.strategies.items():
            ...
            action: Literal["drop", "impute_median", "impute_mean", "impute_mode", "fill_value", "flag"] = spec.get(
                "action", "drop"
            )

            logger.info("Column '%s' has %d nulls -> Applying strategy '%s'", col, null_count, action)
